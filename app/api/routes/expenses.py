...
def list_expenses(db: Session = Depends(get_db), user_id: int = 1):
    rows = db.execute(select(Expense).where(Expense.user_id == user_id).order_by(Expense.id.desc())).scalars().all()
    return rows
